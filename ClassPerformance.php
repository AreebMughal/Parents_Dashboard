   ...
   <header>
        ...
        <div class="navi">
            <nav class="navbar navbar-expand-md  bg-info sticky-top">
                <a class="navbar-brand" href="index.php">
                    <span class=""></span> Class Performance</a>
    ...
    <main>
         <?php 
                ...
                $q = "SELECT * FROM `performance` WHERE `id` = '$id'";
                $run = mysqli_query($con,$q);
                if(mysqli_num_rows($run)!=0){
                    $html .= '
                    <div class="container col-md-6" style="display: flex; margin-top: 20px;padding: 20px">
                            ...
                            <table class="table table-striped table-hover">
                                <tbody>
                                    <tr>
                                        ...
                                        <td class="head">1st term</td>
                                        <td class="head">2st term</td>
                                        <td class="head">Final</td>
                                    </tr>';        
                while ($row = $run->fetch_assoc()){
                    $html.= '<tr>
                            <td class="data">'.$row['Subject'].'</td>
                            <td class="data">'.$row['test1'].'</td>
                            <td class="data">'.$row['test2'].'</td>
                            <td class="data">'.$row['1stTerm'].'</td>
                            <td class="data">'.$row['2ndTerm'].'</td>
                            <td class="data">'.$row['final'].'</td>
                        </tr>';
                } 
            ...
            else
            {
                echo '<div class="marquee container">
                        <marquee scrollamount="8"><h1>There is no Data in DataBase due to some Problems.</h1></marquee>
                    </div></div>';
            }
        ?>
    </main>
